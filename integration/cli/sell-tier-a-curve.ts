// ============================================================================
// Noctis Protocol — Tier A Preprod Milestone, Phase 4 follow-up (T106)
...
// ({txHash, netProceeds, claimedPrice}).
// ============================================================================
...
async function main() {
  ...
  const required: Array<keyof SellTierACurveInput> = [
    'network', 'launchIdHex', 'sellerMnemonic', 'tokenAmount',
  ...
  const NETWORK_MAP: Record<SellTierACurveInput['network'], LucidNetwork> = {
    preview: 'Preview',
    preprod: 'Preprod',
    mainnet: 'Mainnet',
  };
  ...
  const result = await submitter.sellTokens(input.sellerMnemonic, BigInt(input.tokenAmount));
